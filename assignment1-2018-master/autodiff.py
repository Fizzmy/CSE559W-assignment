...
class ReduceSumOp(Op):
    ...
    def gradient(self,node,output_grad):
        return [oneslike_op(node.inputs[0])*output_grad]
...
class DivByConstOp(Op):
    def __call__(self,node_A,const_val):
        new_node = Op.__call__(self)
        new_node.inputs = [node_A]
        new_node.name = "(%s/%s)" % (node_A.name,const_val)
        new_node.const_attr=const_val
        return new_node
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if isinstance(node.op,PlaceholderOp):
                continue
            values=[node_to_val_map[i] for i in node.inputs]
            result = node.op.compute(node,values)
            if isinstance(result,np.ndarray):
                node_to_val_map[node]=result
            else:
                node_to_val_map[node]=np.array(result)
        # Collect node values.
