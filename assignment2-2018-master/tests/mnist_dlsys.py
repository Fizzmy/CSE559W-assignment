...
def mnist_logreg(executor_ctx, num_epochs=10, print_loss_val_each_epoch=False):
    ...
    W1_val = tvm.nd.array(W1_val, device=executor_ctx)
    b1_val = tvm.nd.array(b1_val, device=executor_ctx)
    X_val = tvm.nd.array(X_val, device=executor_ctx)
    y_val = tvm.nd.array(y_val, device=executor_ctx)
    valid_X_val = tvm.nd.array(valid_X_val, device=executor_ctx)
    valid_y_val = tvm.nd.array(valid_y_val, device=executor_ctx)
...
def mnist_mlp(executor_ctx=None, num_epochs=10,
    ...
    W1_val = tvm.nd.array(W1_val, device=executor_ctx)
    W2_val = tvm.nd.array(W2_val, device=executor_ctx)
    W3_val = tvm.nd.array(W3_val, device=executor_ctx)
    b1_val = tvm.nd.array(b1_val, device=executor_ctx)
    b2_val = tvm.nd.array(b2_val, device=executor_ctx)
    b3_val = tvm.nd.array(b3_val, device=executor_ctx)
    X_val = tvm.nd.array(X_val, device=executor_ctx)
    y_val = tvm.nd.array(y_val, device=executor_ctx)
    valid_X_val = tvm.nd.array(valid_X_val, device=executor_ctx)
    valid_y_val = tvm.nd.array(valid_y_val, device=executor_ctx)
