...
def test_matrix_elementwise_add():
    ...
    arr_x = tvm.nd.array(x, device=ctx)
    arr_y = tvm.nd.array(y, device=ctx)
    arr_z = tvm.nd.array(z, device=ctx)
...
def test_conv2d():
    ...
    arr_x = tvm.nd.array(x, device=ctx)
    arr_f = tvm.nd.array(f, device=ctx)
    arr_y = tvm.nd.array(y, device=ctx)
...
def test_relu_gradient():
    ...
    y = np.zeros(shape).astype(dtype)
    arr_x = tvm.nd.array(x, device=ctx)
    arr_grad_x = tvm.nd.array(grad_x, device=ctx)
    arr_y = tvm.nd.array(y, device=ctx)
...
def test_softmax_cross_entropy():
    ...
    arr_y = tvm.nd.array(y, device=ctx)
    arr_y_ = tvm.nd.array(y_, device=ctx)
    arr_out = tvm.nd.array(out, device=ctx)
